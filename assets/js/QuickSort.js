...
async function hoare_partition(l, r, delay = 700) {
    ...
    while (true) {
        ...
        // If two pointers met.
        if (i >= j) {
            for (var k = 0; k < 20; k++) blocks[k].style.backgroundColor = "#00ff00";
            return j;
        }
...
async function QuickSort_HP(l, r, delay = 100) {
    // QuickSort Algorithm
    if (l < r) {
        //Storing the index of pivot element after partition
        var pivot_idx = await hoare_partition(l, r);
        //Recursively calling quicksort for left partition
        await QuickSort_HP(l, pivot_idx);
        //Recursively calling quicksort for right partition
        await QuickSort_HP(pivot_idx + 1, r);
    }
}
...
async function lometo_partition(l, r, delay = 700) {
    ...
    for (var k = 0; k < 20; k++) 
    blocks[k].style.backgroundColor = "#00ff00";
    return i;
  }
    
  // Asynchronous QuickSort function
  async function QuickSort_LP(l, r, delay = 100) {
    if (l < r) {
      // Storing the index of pivot element after partition
      var pivot_idx = await lometo_partition(l, r);
      // Recursively calling quicksort for left partition
      await QuickSort_LP(l, pivot_idx - 1);
      // Recursively calling quicksort for right partition
      await QuickSort_LP(pivot_idx + 1, r);
    }
  }
...
document.getElementById("Button1").style.backgroundColor = "#fab39e";

// To disable the button "Selection Sort"
document.getElementById("Button2").disabled = true;
document.getElementById("Button2").style.backgroundColor = "#fab39e";

document.getElementById("ButtonGenerate").disabled = true;
document.getElementById("ButtonGenerate").style.backgroundColor = "#fab39e";
}
